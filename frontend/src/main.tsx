...
type VideoInput={input_id:string;width:number;height:number;duration_seconds:number;source_fps:number|null;normalized_fps:number;normalized_frames:number;has_audio:boolean};
...
function App(){
 ...
 async function uploadVideo(selected:File|null){
  ...
  try{const result=await api('/video-inputs?output_fps='+encodeURIComponent(outputFps),{method:'POST',headers:{'content-type':'application/octet-stream'},body:selected,signal:controller.signal});setVideoInput(result);refineKey.current=crypto.randomUUID();return result as VideoInput;}
 ...
 <aside>{status?.active_kind==='validate_video'?'動画変換中':status?.runtime_ready?'処理受付可能':`準備中・受付停止：${status?.reason||'確認中'}`}{status?.available_gib!==undefined&&` ／ RAM空き ${status.available_gib.toFixed(1)} GiB`}</aside>
 ...
 <label>入力動画<input type="file" accept="video/mp4,video/quicktime,.mp4,.mov" disabled={busy||running||uploading||!status?.video_validation_ready} onChange={e=>{setVideoFile(e.target.files?.[0]||null);setVideoInput(null);setError('');}}/></label>
 {videoInput?<><p>変換済み入力（{videoInput.normalized_fps}fpsのMP4）</p><video controls src={'/api/video-inputs/'+videoInput.input_id+'/preview'} aria-label="入力動画"/></>:videoPreview&&<><p>処理開始後、変換したMP4を表示します。</p>{videoFile?.type!=='video/quicktime'&&!videoFile?.name.toLowerCase().endsWith('.mov')&&<video controls src={videoPreview} aria-label="入力動画"/>}</>}
 {uploading&&<p role="status">アップロード・動画変換中… <button type="button" onClick={()=>uploadAbort.current?.abort()}>アップロード接続を中断</button></p>}
 {videoInput&&<p>{videoInput.width}×{videoInput.height} ／ {videoInput.duration_seconds}秒 ／ 入力{videoInput.source_fps??'不明'}fps → 出力{videoInput.normalized_fps}fps・{videoInput.normalized_frames}フレーム ／ {videoInput.has_audio?'音声あり':'無音'}</p>}
 <button disabled={busy||running||uploading||(!videoInput&&!videoFile)||!status?.refiner_ready}>高解像度化</button>
